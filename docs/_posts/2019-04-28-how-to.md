...
For the _data binding_ we use [Binding.scala].
  ...
  private def printForm(plotDiv: String): Binding[HTMLElement] = {
    ...
    <div class="ui form">
       ... // a table with the inputs
          <input class="right" type="text" name="radiusIn" id="radiusIn"           placeholder="Radius" value={s"$r"} 
...
We add an `onblur` on the `input` fields, that set the _new radius_ after changing the _radius_ or the _area_.
...
When you see the result - because the whole part is repainted each time - it is not so satisfing. Can we do better?
